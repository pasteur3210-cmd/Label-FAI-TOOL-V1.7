...
'''
    ...
    fusion=MultiImageInspectionEngine(fusion_profile,'1.9.22')
    ...
    check('_golden_review_machine_code_box' in app and '_golden_review_field_key' in app, 'V1.9.22 field-safe machine-code review locator is missing')
    check('factory-review locator: use a callout arrow as' in app and 'draw.polygon([(cx,cy),p1,p2]' in app, 'V1.9.22 arrow/callout Golden locator is missing')
    ...
    # 9) V1.9.22 alignment: CMP-001 shipped-label scope, CMP-002/003 length-only, CMP-008 notch.
    ...
    # 10) V1.9.22 production allocation gates: S/N inclusive range, MAC HEX
    ...
    # 11) V1.9.22 field hardening: GRG-4297u has an earlier shipped WiFi QR
    ...
    check(grg19.get('inspection_scope')=='REFERENCE_ONLY' and not grg19.get('required'),'V1.9.22 GRG later test QR still enters runtime')
    ...
    naming_engine=MultiImageInspectionEngine(naming_profile,'1.9.22')
