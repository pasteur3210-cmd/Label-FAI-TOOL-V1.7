...
def main() -> int:
    ...
    if '_clean_engine_template(base_profile)' not in gp:
        fail('V1.9.22 clean Dynamic Golden engine-template boundary is missing')
    ...
    if 'extract_golden_form_items' not in gp or 'golden_completeness' not in gp:
        fail('V1.9.22 numbered Golden completeness parser is missing')
    if 'STANDARD_LIBRARY' not in gp or 'validation_readiness_errors' not in gp:
        fail('V1.9.22 Standard Library / validation readiness gate is missing')
    if '_show_manual_golden_review' not in app or 'Golden Reference / Golden 對照' not in app:
        fail('V1.9.22 Golden-assisted Manual Review UI is missing')
    if 'Review with Golden / Golden對照復判' not in app:
        fail('V1.9.22 Manual Review button does not visibly expose Golden comparison')
    if '_golden_review_artwork_marker' not in app or 'showing full Golden' not in app or 'artwork_review_roi' not in app:
        fail('V1.9.22 strict Artwork Golden-reference safety/marker rule is missing')
    if 'Full Golden / 完整Golden' not in app or 'Focus Item / 項目放大' not in app:
        fail('V1.9.22 Manual Review full-Golden/default + optional-focus controls are missing')
    if "render_golden(None,'Final Label / Full Golden reference',golden_crop)" not in app:
        fail('V1.9.22 Manual Review does not default to the complete Golden image with current-item highlight')
    if 'REVIEW: {item}' not in app or 'golden_focus_allowed=bool(golden_crop)' not in app:
        fail('V1.9.22 Golden current-item highlight / safe Focus policy is missing')
    if 'Golden Item Specification / Golden 項目說明' not in app or '_golden_item_specification' not in app:
        fail('V1.9.22 item-specific Golden specification panel is missing')
    if 'select_final_label_image' not in gp or 'candidate_layout_policy' not in gp:
        fail('V1.9.22 final-label image selection guard is missing')
    if 'FINAL Label image' not in app or 'support screenshot' not in app:
        fail('V1.9.22 manual review is not isolated from support screenshots')
    if 'golden_item_bindings' not in gp or '_build_golden_item_bindings' not in gp:
        fail('V1.9.22 deterministic inspection-item -> Request-Form binding is missing')
    if 'Numbered Request Form exists => allow only deterministic' not in app:
        fail('V1.9.22 numbered Golden reference still permits fuzzy cross-item lookup')
    if 'urn:schemas-microsoft-com:vml' not in gp or 'imagedata' not in gp:
        fail('V1.9.22 legacy DOC/VML Final Label structural detection is missing')
    if 'final_label_image' not in gp or "gi.get('final_label_image'" not in app:
        fail('V1.9.22 exact persisted Final Label reference is missing')
    ...
    if '_display_name' not in pm or 'label_pn' not in pm:
        fail('V1.9.22 unique Dynamic Profile UI key guard is missing')
    if '_parse_ocr_fields_dynamic' not in parser or 'profile=self.profile' not in (ROOT/'label_tool/core/multi_image_inspection.py').read_text(encoding='utf-8'):
        fail('V1.9.22 profile-aware Dynamic Image parser is missing')
    if 'qr_payload_fields' not in gp or 'ssid_mac_suffix_length' not in gp:
        fail('V1.9.22 Golden runtime rule extraction is missing')
    if 'password_length' not in gp or 'Password\\s*:' not in gp:
        fail('V1.9.22 Golden Password length rule extraction is missing')
    ...
    if 'manual_attention_mode' not in mi or 'manual_reviews' not in mi:
        fail('V1.9.22 all-non-PASS operator-attention/traceability model is missing')
    if 'return bool(str(item or "").strip())' not in mi:
        fail('V1.9.22 every non-PASS item must have a traceable manual PASS path')
    if 'mode=self.multi_image_engine.manual_attention_mode(item)' not in app or 'Confirm PASS / 人工確認PASS' not in app:
        fail('V1.9.22 Manual Review does not expose traceable Confirm PASS for non-PASS items')
    if 'IMAGE_SESSION_INVALIDATED' not in app:
        fail('V1.9.22 profile/Golden change does not invalidate previous Image result')
    if '__incoming_' not in gp or 'backup=root.parent' not in gp:
        fail('V1.9.22 transactional Golden asset replacement guard is missing')
    if '_apply_chassis_scope_filter' not in gp or 'CHASSIS_SHIPPED_LABEL_ONLY' not in gp:
        fail('V1.9.22 CMP-001 shipped Chassis scope filter is missing')
    if '_extract_notch_direction' not in gp or 'Geometry: Label Notch Direction' not in gp:
        fail('V1.9.22 CMP-008 Request-Form notch direction rule is missing')
    if 'detect_label_notch_direction' not in mi or 'GEO-NOTCH-VERIFY' not in mi:
        fail('V1.9.22 CMP-008 runtime notch detector/manual fallback is missing')
    ...
    if 'check_serial_range' not in production or 'check_mac_range' not in production or 'check_mac_allocation' not in production:
        fail('V1.9.22 production S/N/MAC range/allocation engine is missing')
    if 'Check S/N Range' not in app or 'Check MAC Range' not in app or 'Check MAC Allocation Step' not in app:
        fail('V1.9.22 production range GUI controls are missing')
    ...
    for token in ('Work Order: S/N Range','Work Order: MAC Range','Work Order: MAC Allocation Step'):
        if token not in rules:
            fail(f'V1.9.22 production rule missing: {token}')
    if 'DERIVED_RULE_FAIL' not in app:
        fail('V1.9.22 CAM production-range FAIL path is missing')
    if 'prior_shipped_qr' not in gp or 'shipped-label QR is already defined earlier' not in gp:
        fail('V1.9.22 CMP-001 duplicate/test QR scope hardening is missing')
    if '_record_prefix' not in mi or '_sync_named_records' not in mi:
        fail('V1.9.22 traceable session-folder naming is missing')
    for token in ('Request Form','Program Version','Profile Version'):
        if token not in mi:
            fail(f'V1.9.22 report trace metadata missing: {token}')
